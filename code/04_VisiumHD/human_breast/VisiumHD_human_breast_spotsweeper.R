...
spe <- readRDS(here("processed-data", "VisiumHD","VisiumHD_HumanBreast_016.rds"))
spe
...
# ===== Spatially naive QC ======
# plot metrics
png(here(plot_dir, "HD_HumanBreast_016_spotplot_metrics.png"), width=12, height=5, res=300, units="in")
p1 <- plotQC(spe, metric="detected", point_size=0.3)
p2 <- plotQC(spe, metric="sum", point_size=0.3)
p3 <- plotQC(spe, metric="subsets_mito_percent", point_size=0.3)

p1+p2+p3
dev.off()

png(here(plot_dir, "HD_HumanBreast_016_spotplot_metrics_discard.png"), width=12, height=5, res=300, units="in")
p4 <- plotQC(spe,metric="detected", outliers="detected_discard", point_size=0.3, stroke=.5)
p5 <- plotQC(spe, metric="sum", outliers="sum_discard", point_size=0.3, stroke=.5)
p6 <- plotQC(spe,  metric="subsets_mito_percent", outliers="subsets_mito_percent_discard", point_size=0.3, stroke=.5)

p4+p5+p6
dev.off()

# scatter plot of mito and sum
png(here(plot_dir, "HD_HumanBreast_016_scatter_sum_mito.png"), width=5, height=5, res=300, units="in")
...
# violins
png(here(plot_dir, "HD_HumanBreast_016_violins_discarded_spotsweeper.png"), width=9, height=3, res=300, units="in")
...
# spot plots
png(here(plot_dir, "HD_HumanBreast_016_spotplot_discarded_spotsweeper.png"), width=12, height=5, res=1200, units="in")
...
png(here(plot_dir, "HD_HumanBreast_016_spotplot_main_figure.png"), width=12, height=5, res=300, units="in")
