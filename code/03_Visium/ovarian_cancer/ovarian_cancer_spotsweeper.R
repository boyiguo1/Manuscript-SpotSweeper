...
plot_dir <- here("plots","Visium", "ovarian_cancer", "miQC")
...
plotModel(spe, model) + ggtitle("Ovarian Cancer")
dev.off()

png(here(plot_dir, "miQC_mixtureModelFiltering.png"), width=4, height=4, units="in", res=300)
plotFiltering(spe, model) + ggtitle("Ovarian Cancer")
dev.off()
...
saveRDS(spe, here("processed-data", "Visium","ovarian_cancer","OvarianCancer_banksy_spotsweeper.rds"))